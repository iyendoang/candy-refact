...
if($nilai_temp['ujian_selesai'<>""]){
    if ($pengawas['level'] == 'admin') {
        ...
        $nilaiq = mysqli_query($koneksi, "SELECT * FROM nilai_temp s 
        LEFT JOIN ujian c ON s.id_ujian=c.id_ujian 
        LEFT JOIN mapel m ON m.kode=c.kode_nama 
        LEFT JOIN pengawas p ON p.id_pengawas=m.idguru 
        WHERE c.status='1' AND s.id_siswa<>'' AND (p.id_pengawas='$pengawas[id_pengawas]' OR c.id_guru='$_SESSION[id_pengawas]') $queryidu 
        GROUP BY s.id_nilai DESC");
    }
}else{
    if ($pengawas['level'] == 'admin') {
        ...
        $nilaiq = mysqli_query($koneksi, "SELECT * FROM nilai s 
        LEFT JOIN ujian c ON s.id_ujian=c.id_ujian  
        LEFT JOIN mapel m ON m.kode=c.kode_nama 
        LEFT JOIN pengawas p ON p.id_pengawas=m.idguru 
        WHERE c.status='1' AND s.id_siswa<>'' AND (p.id_pengawas='$pengawas[id_pengawas]' OR c.id_guru='$_SESSION[id_pengawas]') $queryidu 
